...
// You can adjust these variables to control the noise
...
void main(){
    ...
    float noiseValue = noise((st + offset) * scale, time);
    noiseValue *= strength;

    // Add color to noise
    vec3 color = vec3(0.1, 0.1, 0.9) + noiseValue * vec3(0.9, 0.9, 0.9);
