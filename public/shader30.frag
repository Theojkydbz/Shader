...
float random(vec2 st) {
    return fract(sin(dot(st.xy, vec2(1324897.43895, 724779.3569))) * 44234.5453);
}
...
vec3 getBlockColor(vec2 st, vec2 blockSize, vec3 lightColor, vec2 blockPos, vec2 cursorPos) {
    vec2 blockCoords = fract(st / blockSize) - 0.5;

    // Calculate unique light position for each block based on cursor position
    float lightSpeed = 42.1;
    vec2 lightOffset = vec2(cos(u_time + blockPos.x * -lightSpeed), sin(u_time + blockPos.y * lightSpeed)) * .2;
    vec2 lightPos = cursorPos + lightOffset;

    float distanceToLight = length(blockCoords - lightPos);
    float lightIntensity = 0.5 / (distanceToLight * distanceToLight);
...
void main() {
    ...
    // Grid properties
    vec2 gridSize = vec2(0.05, 0.05);
    vec2 blockSize = vec2(0.0001, 0.0001);
    ...
    vec3 blockColor = getBlockColor(blockPos, blockSize, lightColor, blockPos, u_cursor);
